...
## ESP (Live Roster OCR)
The in-app feature — **"ESP" is the user-chosen name** — that obtains the **live** match roster by OCR-reading Deadlock's in-game **ESC › PLAYERS panel** (the `MY TEAM` / `ENEMY TEAM` list), because no API exposes a live roster (see [[live-roster-availability]]). A Python child process (`ocr-worker/`) is spawned by the Main process **only while `GAME_IN_MATCH` and the ESP toggle is on**; it reads each player's **hero** (mapped to `hero_id` by fuzzy-match against the heroes asset — closed vocabulary of ~28 names, validated 100% on the reference capture) and **Steam display name** (**best-effort, display-only**: a display name is **not** resolvable to an `account_id`, so it drives **no** per-account stats; OCR also yields **no lane/slot**). Players are split **relative to the local player** (`myTeam` / `enemyTeam`), not API `team` 0/1 — which is exactly what the overlay's [[item-suggestions]] needs (`enemy_hero_ids`). Output is NDJSON on stdout (one roster line per scan), consistent with the existing `python-runner` pattern. **Naming caveat:** distinct from the school acronym and from the gaming term "ESP" (wallhack); here it denotes the screen-reading roster feature only. See [[adr-0005]].
